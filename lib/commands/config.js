...
async function getValues(name) {
  let currentValue = await configs.getRc(name);
  let defaultValue = pkg.configs[name];
  let list = [{
  ...
  let historyValues = (await configs.getLocalConf(name, []))
